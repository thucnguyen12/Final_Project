...
static SemaphoreHandle_t m_sem_usart3_tx;
/* USER CODE END 0 */
...
void MX_USART3_UART_Init(void)
{
  ...
  /* USER CODE BEGIN USART3_Init 0 */
	if (m_sem_usart3_tx == NULL)
	{
		m_sem_usart3_tx = xSemaphoreCreateBinary();
	}
...
void usart_send_bytes(uint32_t uart_addr, uint8_t *data, uint32_t size)
{
    ...
    if ((USART_TypeDef*)uart_addr == USART3)
    {
        ...
        xSemaphoreTake(m_sem_usart3_tx, portMAX_DELAY);
    }
}
...
//    DEBUG_ISR("TX cplt\r\n");
    BaseType_t ctx_sw;
    xSemaphoreGiveFromISR(m_sem_usart3_tx, &ctx_sw);
